<?php
    ...
    class ArithmeticAxiomsTest extends TestCase {
        ...
        public function testDigitalRootAddition(int $a, int $b)
        {
            ...
            $dr⟮dr⟮a⟯＋dr⟮b⟯⟯
                = Arithmetic::digitalRoot(Arithmetic::digitalRoot($a)
                + Arithmetic::digitalRoot($b));
        ...
        public function testModuloIdentity()
        {
            ...
            foreach (range(-20, 20) as $a)
            {
                ...
                {
                    // When
                    $⟮a mod n⟯ mod n = Arithmetic::modulo(Arithmetic::modulo($a,
                        $n), $n);
            ...
            foreach (range(-20, 20) as $a)
            {
                foreach (range(-20, 20) as $n)
                {
                    ...
                    $⟦⟮−a mod n⟯ ＋ ⟮a mod n⟯⟧ mod n = Arithmetic::modulo(
                        Arithmetic::modulo(-$a, $n) + Arithmetic::modulo($a,
                            $n),
                        $n
                    );
                ...
                foreach (range(-5, 5) as $b)
                {
                    foreach (range(-6, 6) as $n)
                    {
                        ...
                        $⟦⟮a mod n⟯ ＋ ⟮b mod n⟧⟯ mod n = Arithmetic::modulo(
                            Arithmetic::modulo($a, $n) + Arithmetic::modulo($b,
                                $n),
                            $n
                        );
