<?php
    ...
    class Eigenvector {
         ...
         *
         * @throws \MathPHP\Exception\BadDataException if the matrix is not square; improper number of eigenvalues;
         ...
         */
        public static function eigenvectors(
            NumericMatrix $A,
            array $eigenvalues = []
        ): NumericMatrix {
            ...
            foreach ($eigenvalues as $eigenvalue)
            {
                ...
                if ( ! $key)
                {
                    ...
                    while ($number_of_solutions > $vectors_found)
                    {
                        ...
                        $number_to_force = $number_of_solutions
                            - $vectors_found;
                        $forced_variables = [];
                        $n = $rref->getN();
                        ...
                        $eigenvector_scaled
                            = Single::multiply($eigenvector_scaled,
                            $scale_factor);
                        $solution_array[] = [
                            'eigenvalue' => $eigenvalue,
                            'vector'     => $eigenvector_scaled,
                        ];
                        $vectors_found++;
