    ...
    class PCA {
        ...
        public function __construct(
            ...
            $corrCovMatrix = $this->data->transpose()->multiply($this->data)
                ->scalarDivide($samples - 1);

            // Eigenvalues and vectors
            $this->EVal
                = new Vector($corrCovMatrix->eigenvalues(Eigenvalue::JACOBI_METHOD));
            $this->EVec
         ...
         *
         * @param \MathPHP\LinearAlgebra\NumericMatrix|null $new_data - An optional Matrix of new data which is standardized against the original data
         *
        ...
        public function standardizeData(NumericMatrix $new_data = NULL
            ...
            $scale_matrix = MatrixFactory::diagonal($this->scale->getVector())
                ->inverse();
         ...
         * @throws \MathPHP\Exception\OutOfBoundsException
         */
        public function getQResiduals(NumericMatrix $new_data = NULL
        ): NumericMatrix {
            $vars = $this->data->getN();
            ...
            $P = $this->EVec->submatrix(0, 0, $vars - 1,
                0);  // Get the first column of the loading matrix
            $P′ = $P->transpose();
            /** @var NumericMatrix $Q */
            $Q
                = MatrixFactory::create([
                $X->multiply($I->subtract($P->multiply($P′)))->multiply($X′)
                    ->getDiagonalElements(),
            ])->transpose();
         ...
         * @throws \MathPHP\Exception\MatrixException
         */
        public function getT2Distances(NumericMatrix $new_data = NULL
        ): NumericMatrix {
            $vars = $this->data->getN();
            ...
            $P′ = $P->transpose();
            $Λⱼ⁻¹
                = MatrixFactory::diagonal(array_slice($this->EVal->getVector(),
                0, 1))->inverse();
            /** @var NumericMatrix $T² */
            $T² = MatrixFactory::create([
                $X->multiply($P)->multiply($Λⱼ⁻¹)->multiply($P′)->multiply($X′)
                    ->getDiagonalElements(),
            ])->transpose();
            ...
            for ($i = 1; $i < $this->data->getN(); $i++)
            {
                ...
                $Tᵢ² = MatrixFactory::create([
                    $X->multiply($P)->multiply($Λⱼ⁻¹)->multiply($P′)
                        ->multiply($X′)->getDiagonalElements(),
                ])->transpose();
                $T² = $T²->augment($Tᵢ²);
            }
        ...
        public function getCriticalQ(float $alpha = .05): array
        {
            ...
            for ($i = 0; $i < $vars - 1; $i++)
            {
                $evals = array_slice($this->getEigenvalues()->getVector(),
                    $i + 1);
