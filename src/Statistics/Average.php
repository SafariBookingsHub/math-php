<?php
    ...
    class Average {
         ...
         * @throws Exception\BadDataException if the input array of numbers is empty
         */
        public static function truncatedMean(
            array $numbers,
            int $trim_percent
        ): float {
            if (empty($numbers))
         ...
         */
        public static function simpleMovingAverage(
            array $numbers,
            int $n
        ): array {
            $m = count($numbers);
         ...
         */
        public static function weightedMovingAverage(
            array $numbers,
            int $n,
            array $weights
        ): array {
            ...
            for ($i = 0; $i <= $m - $n; $i++)
            {
                $∑wp = array_sum(Map\Multi::multiply(array_slice($numbers, $i,
                    $n), $weights));
                $WMA[] = $∑wp / $∑w;
            }
         ...
         */
        public static function exponentialMovingAverage(
            array $numbers,
            int $n
        ): array {
            $m = count($numbers);
         ...
         * @return float
         */
        public static function arithmeticGeometricMean(
            float $x,
            float $y
        ): float {
            // x or y < 0 = NaN
        ...
        public static function mode(array $numbers): array
        {
            ...
            $array_map1 = [];
            foreach ($numbers as $key => mixed$number_strings = $array_map1;
            ...
            $array_map = [];foreach ($modes as $key => mixedreturn $array_map;
        }
