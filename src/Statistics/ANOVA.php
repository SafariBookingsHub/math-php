    ...
    class ANOVA {
        ...
        public static function oneWay(array ...$samples): array
        {
            ...
            foreach ($samples as $i => $sample)
            {
                ...
                $summary_data[$i]['variance']
                    = Descriptive::sampleVariance($sample);
                $summary_data[$i]['sd'] = Descriptive::sd($sample);
                $summary_data[$i]['sem']
                    = RandomVariable::standardErrorOfTheMean($sample);
            }
            ...
            {
                $array_map1[$key] = $n * (Average::mean($sample) - $μ) ** 2;
            }
            ...
            {
                $array_map[$key]
                    = MathPHP\Statistics\RandomVariable::sumOfSquaresDeviations($samples[$key]);
            }
        ...
        public static function twoWay(array ...$data): array
        {
            ...
            foreach ($data as $A => $Bs)
            {
                ...
                foreach ($Bs as $B => $values)
                {
                    ...
                    $summary_AB[$A][$B]['SS']
                        = RandomVariable::sumOfSquares($values);
                    $summary_AB[$A][$B]['variance']
                        = Descriptive::sampleVariance($values);
                    $summary_AB[$A][$B]['sd'] = Descriptive::sd($values);
                    $summary_AB[$A][$B]['sem']
                        = RandomVariable::standardErrorOfTheMean($values);
                }
            }
            ...
            foreach ($A_elements as $A => $elements)
            {
                ...
                $summary_A[$A]['variance']
                    = Descriptive::sampleVariance($elements);
                $summary_A[$A]['sd'] = Descriptive::sd($elements);
                $summary_A[$A]['sem']
                    = RandomVariable::standardErrorOfTheMean($elements);
            }
            ...
            {
                $array_map1[$key] = $f1['n'] * ($f1['mean'] - $μ) ** 2;
            }
